...
class ScreenStateViewBuilder {
  ...
  Widget buildErrorView(BuildContext context, String errorDescription, Future Function({bool? showLoading})? refresh) =>
      errorViewBuilder?.call(context, errorDescription) ??
      ErrorView(errorDescription: errorDescription, refresh: refresh != null ? () => refresh(showLoading: true) : null);

  Widget buildLoadingView(BuildContext context) => loadingViewBuilder?.call(context) ?? const LoadingIndicator();

  Widget buildPopulatedView(BuildContext context, Widget body) => populatedViewBuilder?.call(context, body) ?? body;

  Widget buildEmptyView(BuildContext context, Future Function({bool? showLoading})? refresh) =>
      emptyViewBuilder?.call(context, refresh) ?? EmptyStateView(refresh: refresh);

  showAlert(BuildContext context, String message) => WidgetsBinding.instance.addPostFrameCallback((_) {
